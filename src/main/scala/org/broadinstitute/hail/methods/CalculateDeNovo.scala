...
object CalculateDeNovo {
  ...
  val HEADER = Array("Chr", "Pos", "rsID", "Ref", "Alt", "Proband_ID", "Father_ID",
  ...
  def apply(vds: VariantDataset, preTrios: Array[CompleteTrio],
    popFreqFn: (Annotation) => Option[Double],
    additionalQueries: Option[(EvalContext, Array[(() => Option[Any])])]): RDD[String] = {
        ...
        (0 until nTrios)
          ...
          .flatMap { i =>
            ...
            genotypeAnnotation.map { identifier =>
              ...
              sb.append(v.start)
              sb += '\t'
              sb.append(rsidQuerier(va).getOrElse("."))
              sb += '\t'
              sb.append(v.ref)
              sb += '\t'
              sb.append(v.alt)
