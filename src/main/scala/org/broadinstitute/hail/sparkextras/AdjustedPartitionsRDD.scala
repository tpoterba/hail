...
class AdjustedPartitionsRDD[T](@transient var prev: RDD[T], adjustInfo: IndexedSeq[Adjustment[T, Int]])(implicit tct: ClassTag[T])
  extends RDD[T](prev.sparkContext, Nil) {
  require(adjustInfo.forall(adj => adj.takeWhile.forall { case (it, _) =>
  ...
  override def getPartitions: Array[Partition] = {
    ...
    Array.tabulate(adjustInfo.length) { i =>
      AdjustedPartitionsRDDPartition(i, parentPartitions(i), adjustInfo(i).map(parentPartitions))
    }
  }
  ...
  override def getDependencies: Seq[Dependency[_]] = Seq(new NarrowDependency[T](prev) {
    override def getParents(partitionId: Int): Seq[Int] = {
      val adj = adjustInfo(partitionId)
