...
object OrderedRDDIterator {
  def apply[PK, K, V](rdd: OrderedRDD[PK, K, V], cachedPartitions: Array[Partition],
    context: TaskContext, k0: K): OrderedRDDIterator[PK, K, V] = {
    val it = new OrderedRDDIterator(rdd, cachedPartitions, context)
...
class OrderedRDDIterator[PK, K, V](
  ...
  def setPartition(newPartIndex: Int) {
    ...
    if (partIndex < nPartitions) {
      it = rdd.iterator(cachedPartitions(partIndex), context).buffered
...
class OrderedLeftJoinRDD[PK, K, V1, V2](left: OrderedRDD[PK, K, V1], right: OrderedRDD[PK, K, V2])
  ...
  val cachedRightPartitions = right.partitions
  ...
  override def compute(split: Partition, context: TaskContext): Iterator[(K, (V1, Option[V2]))] = {
    ...
    if (leftIt.isEmpty)
      ...
      val rightIt = OrderedRDDIterator(right, cachedRightPartitions, context, leftIt.head._1)
