...
object OrderedRDD {
  ...
  def coerce[PK, K, V](rdd: RDD[(K, V)], fastKeys: Option[RDD[K]] = None)
    ...
    if (partitionsSorted) {
      val (adjustments, rangeBounds) = rangesAndAdjustments[PK, K, V](sortedKeyInfo)
      val partitioner = OrderedPartitioner[PK, K](rangeBounds)
      ...
      val adjustedRDD = new AdjustedPartitionsRDD(reorderedPartitionsRDD, adjustments)
...
class OrderedRDD[PK, K, V] private(rdd: RDD[(K, V)], val orderedPartitioner: OrderedPartitioner[PK, K])
  ...
  override def coalesce(maxPartitions: Int, shuffle: Boolean = false)(implicit ord: Ordering[(K, V)] = null): RDD[(K, V)] = {
    ...
    val newRangeBounds = Array.tabulate(newPartEnd.length - 1) { i => orderedPartitioner.rangeBounds(newPartEnd(i)) }
