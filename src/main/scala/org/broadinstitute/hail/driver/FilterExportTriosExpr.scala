...
object FilterExportTriosExpr extends Command {
  ...
  def run(state: State, options: Options): State = {
    ...
    val (header, functions) = if (exportArgs.endsWith(".columns")) {
      val (h, fs) = ExportTSV.parseColumnsFile(ec, exportArgs, vds.sparkContext.hadoopConfiguration)
      (Some(h), fs)
    } else {
      val (h, fs) = Parser.parseExportArgs(exportArgs, ec)
      (h, fs)
    }
    val nFn = functions.size
        ...
        (0 until nTrios)
          .flatMap { i =>
            ...
            if (Filter.keepThis(predicate(), keep)) {
              (0 until nFn).foreachBetween(i => sb.tsvAppend(functions(i)()))(() => sb += '\t')
              Some(sb.result)
            }
            else None
          }
