...
class OrderedPartitioner[K: Ordering : ClassTag, V](
  ...
  var rangeBounds: Array[K] = {
    if (partitions <= 1) {
      ...
      val (numItems, sketched) = OrderedPartitioner.sketch(rdd.map(_._1), sampleSizePerPartition)
      if (numItems == 0L) {
        Array.empty
      } else {
        ...
        OrderedPartitioner.determineBounds(candidates, partitions)
      }
    }
  }
...
private[spark] object OrderedPartitioner {
