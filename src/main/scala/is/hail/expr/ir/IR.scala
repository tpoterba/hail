...
sealed trait Typed extends IR {
  def pType: PType = CanonicalPType(typ)
}

sealed trait PTyped extends IR {
...
final case class Literal(typ: Type, value: Annotation, id: String) extends IR with Typed {
  require(id.startsWith("_"))
}

// TODO: should infer pType
sealed trait InferIR extends Typed {
  var _typ: Type = null
...
final case class I32(x: Int) extends PTyped { val pType = PCanonicalInt32 }
final case class I64(x: Long) extends PTyped { val pType = PCanonicalInt64 }
final case class F32(x: Float) extends PTyped { val pType = PCanonicalFloat32 }
final case class F64(x: Double) extends PTyped { val pType = PCanonicalFloat64 }
final case class Str(x: String) extends PTyped { val pType = PCanonicalString }
final case class True() extends PTyped { val pType = PCanonicalBoolean }
final case class False() extends PTyped { val pType = PCanonicalBoolean }
final case class Void() extends PTyped { val pType = PVoid }

final case class Cast(v: IR, typ: Type) extends Typed

final case class NA(typ: Type) extends Typed { assert(!typ.required) }
final case class IsNA(value: IR) extends PTyped { val pType = PCanonicalBoolean }
...
final case class Ref(name: String, var typ: Type) extends Typed
...
final case class MakeArray(args: Seq[IR], typ: TArray) extends IR {
  val pType: PArray = PCanonicalArray(PType.canonical(typ.elementType), typ, false)
}
final case class ArrayRef(a: IR, i: IR) extends InferIR
final case class ArrayLen(a: IR) extends PTyped { val pType = PCanonicalInt32 }
final case class ArrayRange(start: IR, stop: IR, step: IR) extends PTyped { val pType  = PCanonicalArray(PCanonicalInt32, TArray(TInt32()), elementsRequired = false) }
...
final case class LowerBoundOnOrderedCollection(orderedCollection: IR, elem: IR, onKey: Boolean) extends PTyped { val pType = PCanonicalInt32 }
...
final case class ArrayFor(a: IR, valueName: String, body: IR) extends Typed {
  val typ = TVoid
}
...
final case class InitOp(i: IR, args: IndexedSeq[IR], aggSig: AggSignature) extends Typed {
  val typ = TVoid
}
final case class SeqOp(i: IR, args: IndexedSeq[IR], aggSig: AggSignature) extends Typed {
  val typ = TVoid
}

final case class Begin(xs: IndexedSeq[IR]) extends Typed {
  val typ = TVoid
}
...
final case class StringSlice(s: IR, start: IR, end: IR) extends PTyped {
  val pType = PCanonicalString
}
final case class StringLength(s: IR) extends PTyped {
  val pType = PCanonicalInt32
}

final case class In(i: Int, typ: Type) extends Typed
// FIXME: should be type any
final case class Die(message: String, typ: Type) extends Typed

final case class ApplyIR(function: String, args: Seq[IR], conversion: Seq[IR] => IR) extends Typed {
  lazy val explicitNode: IR = conversion(args)

  def typ: Type = explicitNode.typ
}

sealed abstract class AbstractApplyNode[F <: IRFunction] extends Typed {
  def function: String
  def args: Seq[IR]
...
final case class Uniroot(argname: String, function: IR, min: IR, max: IR) extends PTyped { val pType = PCanonicalFloat64 }

final case class TableCount(child: TableIR) extends PTyped { val pType = PCanonicalInt64 }
...
final case class TableWrite(
  ...
  codecSpecJSONStr: String = null) extends Typed {
  val typ: Type = TVoid
}
final case class TableExport(
  ...
  header: Boolean = true,
  exportType: Int = ExportType.CONCATENATED) extends Typed {
  val typ: Type = TVoid
}

final case class MatrixWrite(
  child: MatrixIR,
  f: (MatrixValue) => Unit) extends Typed {
  val typ: Type = TVoid
}
