...
class MatrixTable(val hc: HailContext, val ast: MatrixIR) extends JoinAnnotator {
  ...
  def keyRowsBy(keys: Array[String], partitionKeys: Array[String]): MatrixTable = {
    ...
    assert(keys.forall(rowFields.contains))
  ...
  def annotateVariantsExpr(expr: String): MatrixTable = {
    ...
    val newMatrixType = matrixType.copy2(rowType = newRowType)
  ...
  def orderedRVDLeftJoinDistinctAndInsert(right: OrderedRVD, root: String, product: Boolean): MatrixTable = {
    ...
    val makeRightValues = rightRVD.typ.makeValueStruct()
    ...
    val emptyResult: IndexedSeq[Any] = Array.empty[Any]
    val localEntriesIndex = entriesIndex
        ...
        leftRVD.orderedJoinDistinct(rightRVD, "left")
          .mapPartitions { it =>
            ...
            it.map { jrv =>
              ...
              val rightValue = if (jrv.rvRight != null) {
                val r = makeRightValues(jrv.rvRight)
                if (product) {
                  assert(r.length == 1)
                  r.get(0)
                } else r
              } else if (product) emptyResult
              else null

              rvb.set(lrv.region)
              rvb.start(newRVType)
              ins(lrv.region, lrv.offset, rvb,
                () => rvb.addAnnotation(valueType, rightValue))
  ...
  def selectCols(fields: java.util.ArrayList[String]): MatrixTable = selectCols(fields.asScala.toArray: _*)

  def selectCols(fields: String*): MatrixTable = {
    // FIXME: should support nested fields
    val fieldsToKeep = fields.toArray
    val colFieldSet = colType.fieldNames.toSet
    val keepSet = fieldsToKeep.toSet
    assert(fieldsToKeep.forall(colFieldSet.contains))

    val keepIndices = fieldsToKeep.map(colType.fieldIdx)

    val newColType = TStruct(keepIndices.zip(fieldsToKeep).map { case (i, f) => f -> colType.fieldType(i) }: _*)
    val newCK = colKey.filter(keepSet.contains)

    val newMatrixType = matrixType.copy(
      colType = newColType,
      colKey = newCK)

    val newColValues = colValues.map { a =>
      val r = a.asInstanceOf[Row]
      Row.fromSeq(keepIndices.map(r.get))
    }.toArray

    copyMT(matrixType = newMatrixType, colValues = newColValues)
  }

  def selectRows(fields: java.util.ArrayList[String]): MatrixTable = selectRows(fields.asScala.toArray: _*)

  def selectRows(fields: String*): MatrixTable = {
    // FIXME: should support nested fields
    val fieldsToKeep = fields.toArray
    val rowFieldSet = rowType.fieldNames.toSet
    val keepSet = fieldsToKeep.toSet
    assert(fieldsToKeep.forall(rowFieldSet.contains))

    val oldRVType = rvRowType
    val keepIndices = fieldsToKeep.map(oldRVType.fieldIdx)

    val newRowType = TStruct(keepIndices.zip(fieldsToKeep).map { case (i, f) => f -> rowType.fieldType(i) }: _*)
    val (newRK, newRPK) = if (rowKey.forall(keepSet.contains))
      (rowKey, rowPartitionKey)
    else
      (rowKey.filter(keepSet.contains), rowPartitionKey.filter(keepSet.contains))

    val newMatrixType = matrixType.copy2(
      rowType = newRowType,
      rowKey = newRK,
      rowPartitionKey = newRPK)

    val localEntriesIndex = entriesIndex
    val newRVType = newMatrixType.rvRowType
    val mapPartsF: Iterator[RegionValue] => Iterator[RegionValue] = { it =>

      val rv2 = RegionValue()
      val rvb = new RegionValueBuilder()
      //      val fullRow = new UnsafeRow(oldRVType)
      ...
      it.map { rv =>
        ...
        while (i < keepIndices.length) {
          rvb.addField(oldRVType, rv, keepIndices(i))
          i += 1
        }
        rvb.addField(oldRVType, rv, localEntriesIndex)
        rvb.endStruct()
        rv2.set(rv.region, rvb.end())
        rv2
      }
    ...
    if (rowKey == newRK)
      copyMT(matrixType = newMatrixType,
        rvd = rvd.mapPartitionsPreservesPartitioning(newMatrixType.orderedRVType)(mapPartsF))
    else // reorder if keys changed
      copyMT(matrixType = newMatrixType,
        rvd = OrderedRVD(newMatrixType.orderedRVType, rvd.mapPartitions(mapPartsF), None, None))
  }

  def selectEntries(fields: java.util.ArrayList[String]): MatrixTable = selectEntries(fields.asScala.toArray: _*)

  def selectEntries(fields: String*): MatrixTable = {
    // FIXME: should support nested fields
    val fieldsToKeep = fields.toArray
    val entryFieldSet = entryType.fieldNames.toSet
    val keepSet = fieldsToKeep.toSet
    assert(fieldsToKeep.forall(entryFieldSet.contains))

    val keepIndices = fieldsToKeep.map(entryType.fieldIdx)

    val newEntryType = TStruct(keepIndices.zip(fieldsToKeep).map { case (i, f) => f -> entryType.fieldType(i) }: _*)
    ...
    rewriteEntries(noOp)(newEntryType, { case (_, rv, rvb) =>
      val entriesOffset = localRVType.loadField(rv, localEntriesIndex)
      ...
      while (i < localNCols) {
        if (localEntriesType.isElementMissing(rv.region, entriesOffset, i))
          ...
          val eltOffset = localEntriesType.loadElement(rv.region, entriesOffset, i)
          rvb.startStruct()
          var j = 0
  ...
  def annotateVariantsVDS(right: MatrixTable, root: String): MatrixTable = {
    ...
    orderedRVDLeftJoinDistinctAndInsert(right.rvd, root, product = false)
  }
  ...
  def explodeSamples(code: String): MatrixTable = {
    ...
    rewriteEntries(noOp, newColType = newColType,
  ...
  def rewriteEntries[PC](makePartitionContext: () => PC, newColType: TStruct = colType,
  ...
  def insertIntoRow[PC](makePartitionContext: () => PC, newColType: TStruct = colType,
    ...
    val (newRVType, ins) = localRVType.unsafeStructInsert(typeToInsert, List(path))
  ...
  def unionCols(right: MatrixTable): MatrixTable = {
    ...
    val newMatrixType = matrixType.copy2() // move entries to the end
  ...
  def queryVariants(exprs: Array[String]): Array[(Annotation, Type)] = {
    ...
    val (zVal, seqOp, combOp, resOp) = Aggregators.makeFunctions[RegionValue](ec, { case (ec, rv) =>
      ...
      val row = ur.delete(localEntriesIndex)
      ec.set(1, row)
    })
  ...
  override def toString =
    s"MatrixTable(rdd2=$rvd, nSamples=$numCols, vaSignature=$rowType, saSignature=$colType, globalSignature=$globalType, sampleAnnotations=$colValues, sampleIdsAndAnnotations=$colValues, globalAnnotation=$globals)"
  ...
  def variantsKT(): Table = {
    ...
    val localRVType = rvRowType
    val localEntriesIndex = entriesIndex
    val df = new Table(hc, rvd.mapPartitions { it =>
      ...
      it.flatMap { rv =>
        ...
        (0 until localNSamples).iterator
          ...
          .map { i =>
            ...
            var j = 0
            while (j < localRVType.size) {
              if (j != localEntriesIndex)
                rv2b.addField(localRVType, rv, j)
              j += 1
            }
  ...
  def unfilterEntries(): MatrixTable = {
    ...
    rewriteEntries(noOp)(localEntryType, { case (_, rv, rvb) =>
      val entriesOffset = localRVType.loadField(rv, localEntriesIndex)
  ...
  def write(dirname: String, overwrite: Boolean = false): Unit = {
    ...
    val partitionCounts = rvd.rdd.writeRows(dirname, rvRowType)
  ...
  def trioMatrix(pedigree: Pedigree, completeTrios: Boolean): MatrixTable = {
    ...
    rewriteEntries(noOp, newColType = newColType, newColKey = Array("id"),
  ...
  def writeKeyedBlockMatrix(dirname: String, expr: String, blockSize: Int = BlockMatrix.defaultBlockSize,
    ...
    if (keepRowKeys)
      dMatrixRowKeys().write(sparkContext, dirname + "/rowkeys")
  ...
  def filterAlleles(filterExpr: String, variantExpr: String = "",
    ...
    if (!entryType.isOfType(Genotype.htsGenotypeType))
      fatal(s"filter_alleles: genotype_schema must be the HTS genotype schema, found: ${ entryType }")

    val genotypeExpr = if (subset) {
      """
g = let newpl = if (isDefined(g.PL))
        let unnorm = range(triangle(newAlleles.length())).map(newi =>
