...
final class Region() extends NativeBase() {
  ...
  final def appendBinary(v: Array[Byte]): Long = {
    ...
    val grain = if (PCanonicalBinary.contentAlignment < 4) 4 else PCanonicalBinary.contentAlignment
  ...
  def visit(t: PType, off: Long, v: ValueVisitor) {
    t match {
      case t: PBoolean => v.visitBoolean(loadBoolean(off))
      case t: PInt32 => v.visitInt32(loadInt(off))
      case t: PInt64 => v.visitInt64(loadLong(off))
      case t: PFloat32 => v.visitFloat32(loadFloat(off))
      case t: PFloat64 => v.visitFloat64(loadDouble(off))
      case t: PString =>
        val boff = off
        v.visitString(t.loadString(this, boff))
      case t: PBinary =>
        val boff = off
        val length = t.loadLength(this, boff)
        val b = loadBytes(t.bytesOffset(boff), length)
        v.visitBinary(b)
      case t: PArray =>
        val aoff = off
        val length = t.loadLength(this, aoff)
        v.enterArray(t.virtualType, length)
        var i = 0
        while (i < length) {
          v.enterElement(i)
          if (t.isElementDefined(this, aoff, i))
            ...
            v.visitMissing(t.elementType.virtualType)
          i += 1
        }
        v.leaveArray()
      case t: PStruct =>
